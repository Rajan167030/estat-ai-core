...
export const Route = createFileRoute("/partners/$partnerId")({
  ...
  head: ({ params }) => ({
    meta: [
      ...
      { name: "description", content: "Partner profile: KYC, lead attribution history, site visits, bookings, revenue and commission standing." },
      { property: "og:title", content: `Channel partner ${params.partnerId} — Estatum ERP` },
      { property: "og:description", content: "Partner KYC, attribution history and commission ledger." },
  ...
  notFoundComponent: () => (
    <AppShell>
      <EmptyState
        ...
        action={<Link to="/partners" className="text-sm font-semibold text-primary">Back to partners</Link>}
      />
    </AppShell>
  ),
});
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<Button variant="outline" size="sm" asChild><Link to="/partners">All partners</Link></Button>}
      />

      <div className="mt-4 flex flex-wrap gap-2">
        <StatusBadge status={p.tier} />
        <StatusBadge status={`KYC ${p.kyc}`} tone={p.kyc === "Verified" ? "green" : p.kyc === "Pending" ? "amber" : "red"} />
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        <SectionCard title="Attribution flow" description="Register → Duplicate check → Attribution → Visit → Booking → Commission">
          <ol className="space-y-2 text-sm">
            {["Lead registered", "Duplicate check", "Attribution locked", "Site visit", "Booking", "Commission"].map((s, i) => (
              <li key={s} className="flex items-center gap-3">
                <span className="num flex size-6 items-center justify-center rounded-full border border-border text-xs">{i + 1}</span>
            ...
            <div className="mt-4 rounded-md border border-warning/25 bg-warning-soft p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <Button size="sm" variant="outline" asChild>
                  <Link to="/leads/$leadId" params={{ leadId: duplicate.id }}>View existing lead</Link>
                </Button>
                <Button size="sm" onClick={() => toast.success("Attribution review requested from Sales Head.")}>
          ...
          <div className="mt-4">
            <div className="mb-1 flex items-center justify-between text-xs text-muted-foreground">
              <span>Conversion</span><span className="num font-semibold text-foreground">{conversion.toFixed(1)}%</span>
            </div>
        ...
        <div className="space-y-4 lg:col-span-2">
          <SectionCard title="Lead history" description="Latest leads registered by this partner" bodyClassName="p-0">
            {partnerLeads.length === 0 ? (
              <div className="p-4"><p className="text-sm text-muted-foreground">No leads registered yet.</p></div>
            ) : (
              <DataTable head={<><Th>Lead</Th><Th>Project</Th><Th className="text-right">Score</Th><Th>Status</Th><Th>Next follow-up</Th></>}>
                {partnerLeads.map((l) => (
                  <tr key={l.id} className="hover:bg-muted/50">
                    <Td>
                      <Link to="/leads/$leadId" params={{ leadId: l.id }} className="font-medium text-primary">{l.name}</Link>
                    </Td>
          ...
          <SectionCard title="Bookings & commission" description="Closed deals and payout standing" bodyClassName="p-0">
            <DataTable head={<><Th>Booking</Th><Th>Unit</Th><Th className="text-right">Value</Th><Th className="text-right">Commission</Th><Th>Status</Th></>}>
              {partnerBookings.map((b) => {
                const c = partnerCommissions.find((x) => x.bookingId === b.id);
                return (
                  <tr key={b.id} className="hover:bg-muted/50">
                    <Td>
                      <Link to="/bookings/$bookingId" params={{ bookingId: b.id }} className="font-medium text-primary">#{b.id}</Link>
                    </Td>
                    ...
                    <Td>{c ? <StatusBadge status={c.status} /> : <StatusBadge status={b.status} />}</Td>
                  </tr>
                );
              })}
            </DataTable>
