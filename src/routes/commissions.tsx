...
export const Route = createFileRoute("/commissions")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Channel partner commissions: raised, pending approval, payable, paid and disputed, with approver on every line." },
      { property: "og:title", content: "Commissions — Estatum ERP" },
      { property: "og:description", content: "Commission ledger with approval trail for every payout." },
...
function CommissionsPage() {
  ...
  return (
    <AppShell>
      <PageHeader title="Commissions" subtitle="Every payout carries an approver and an audit trail" />

      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <KpiCard label="Total commission" value={inr(commissions.reduce((s, c) => s + c.amount, 0))} icon={Percent} accent />
      ...
      <Tabs value={tab} onValueChange={setTab} className="mt-6">
        ...
        <TabsContent value={tab} className="mt-4">
          {rows.length === 0 ? (
            <EmptyState icon={Percent} title="Nothing in this bucket" description="No commission lines currently carry this status." />
          ) : (
            <DataTable
              head={<><Th>Commission</Th><Th>Partner</Th><Th>Booking</Th><Th>Unit</Th><Th className="text-right">Rate</Th><Th className="text-right">Amount</Th><Th>Approver</Th><Th>Raised</Th><Th>Status</Th></>}
            >
              {rows.map((c) => (
                <tr key={c.id} className="hover:bg-muted/50">
                  <Td className="num font-medium">{c.id}</Td>
                  <Td>
                    <Link to="/partners/$partnerId" params={{ partnerId: c.partnerId }} className="font-medium text-primary">
                      {partnerName(c.partnerId)}
                    </Link>
                  </Td>
                  <Td>
                    <Link to="/bookings/$bookingId" params={{ bookingId: c.bookingId }} className="text-primary">#{c.bookingId}</Link>
                  </Td>
