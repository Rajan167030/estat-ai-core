  ...
  head: () => ({
    meta: [
      { title: "AI Calling Agent — Estatum ERP" },
      {
        name: "description",
        content:
          "Run AI voice campaigns that call leads end to end — live queue, transcripts, outcomes and human handoff.",
      },
      { property: "og:title", content: "AI Calling Agent — Estatum ERP" },
      {
        property: "og:description",
        content:
          "AI calls your leads, you approve every script and escalation. Live queue, transcripts and outcomes in one place.",
      },
...
const STATE_TO_STATUS: Record<CallResult["state"], CallStatus> = {
  QUEUED: "Queued",
  COMPLIANCE_BLOCKED: "Failed",
  DIALING: "Dialling",
  NO_ANSWER: "No answer",
  FAILED: "Failed",
  CONNECTED: "In progress",
  AI_CONVERSATION: "In progress",
  HUMAN_HANDOFF: "Completed",
  WRAP_UP: "Completed",
  DONE: "Completed",
};

function toCallRecord(r: CallResult, attempt: number): CallRecord {
  return {
    id: r.id,
    ...
    attempt,
    summary: r.complianceBlockedReason ?? r.outcome ?? r.state,
    nextAction: r.handoffReason ?? (r.state === "HUMAN_HANDOFF" ? "Awaiting human pickup" : "—"),
    transcript: r.transcript,
    objections: [],
    ...(r.outcome ? { outcome: r.outcome } : {}),
    ...(r.sentiment ? { sentiment: r.sentiment } : {}),
    ...(r.handoffTo ? { handoffTo: r.handoffTo } : {}),
  };
}
...
function CallRow({ c }: { c: CallRecord }) {
  return (
    <tr className="transition-colors hover:bg-muted/50">
      ...
      <Td>
        <span className="num text-sm font-semibold">{c.intentScore}</span>
      </Td>
      <Td>
        {c.outcome ? (
          <StatusBadge status={c.outcome} tone={OUTCOME_TONE[c.outcome]} />
        ) : (
          <StatusBadge status={c.status} tone="blue" />
        )}
      </Td>
      <Td>
        {c.sentiment ? (
          <StatusBadge
            status={c.sentiment}
            tone={
              c.sentiment === "Positive" ? "green" : c.sentiment === "Negative" ? "red" : "neutral"
            }
          />
        ) : (
          "—"
      ...
      <Td>
        <Button asChild variant="ghost" size="sm">
          <Link to="/calling-agent/$callId" params={{ callId: c.id }}>
            Open
          </Link>
        </Button>
      </Td>
...
function CallingAgentPage() {
  ...
  const filtered = useMemo(
    () =>
      [...demoCalls, ...callHistory].filter(
        (c) =>
          (campaign === "all" || c.campaignId === campaign) &&
          (outcome === "all" || c.outcome === outcome) &&
          (q === "" ||
            c.leadName.toLowerCase().includes(q.toLowerCase()) ||
            c.id.toLowerCase().includes(q.toLowerCase())),
      ),
    [q, campaign, outcome, demoCalls],
  );

  async function handleRunDemo() {
    setRunningDemo(true);
    try {
      const { completed } = await runDemoCampaign({ data: { count: 3 } });
      setDemoCalls((prev) => [...completed.map((r) => toCallRecord(r, 1)), ...prev]);
      const blocked = completed.filter((r) => r.state === "COMPLIANCE_BLOCKED").length;
      toast.success(
        `Demo campaign done: ${completed.length} calls run${blocked ? ` (${blocked} blocked by compliance)` : ""} via ${completed.length ? "the mock telephony provider" : "no eligible leads"}.`,
      );
    } catch (err) {
    ...
    <AppShell>
      <div className="space-y-5">
          ...
          actions={
            <>
              <div className="flex items-center gap-2 rounded-md border border-border bg-card px-3 py-1.5">
                <Radio
                  className={cn("size-4", autoDial ? "text-success" : "text-muted-foreground")}
                />
                ...
                <Switch
                  ...
                  onCheckedChange={(v) => {
                    ...
                    toast(
                      v
                        ? "Auto-dial resumed for running campaigns."
                        : "Auto-dial paused. Queued calls will hold.",
                    );
                  }}
                />
              </div>
              <Button variant="outline" size="sm" onClick={() => toast("Script library opened.")}>
                Scripts
              </Button>
              <Button variant="outline" size="sm" onClick={handleRunDemo} disabled={runningDemo}>
                {runningDemo ? (
                  <Loader2 className="size-4 animate-spin" />
                ) : (
                  <Sparkles className="size-4" />
                )}{" "}
                Run AI demo campaign
              </Button>
              <Button
                size="sm"
                onClick={() =>
                  toast.success("New campaign draft created — awaiting Sales Manager approval.")
                }
              >
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <KpiCard
            label="Calls today"
            value={num(callingTotals.callsToday)}
            change={14.2}
            icon={PhoneCall}
            accent
          />
          <KpiCard
            label="Connect rate"
            value={`${callingTotals.connectRate}%`}
            change={3.1}
            icon={PhoneForwarded}
          />
          <KpiCard
            label="Avg. talk time"
            value={mmss(callingTotals.avgDurationSec)}
            change={-2.4}
            icon={Timer}
          />
          <KpiCard
            label="Site visits booked"
            value={num(callingTotals.siteVisitsBooked)}
            change={9.6}
            icon={UserCheck}
          />
          <KpiCard
            label="Human handoffs"
            value={num(callingTotals.humanHandoffs)}
            hint="escalated to sales exec"
            icon={PhoneForwarded}
          />
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <SectionCard
            ...
            action={
              <Button
                variant="outline"
                size="sm"
                onClick={() => toast("Queue paused. In-progress calls will finish.")}
              >
                ...
                <li key={c.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
                  ...
                  <div className="min-w-40 flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {projectName(c.projectId)} · attempt {c.attempt} · score {c.intentScore}
                    </p>
                  </div>
                  <StatusBadge
                    status={c.status}
                  ...
                  />
                  <span className="num w-12 text-right text-sm text-muted-foreground">
                    {c.durationSec ? mmss(c.durationSec) : "—"}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() =>
                      toast.success(
                        `Call ${c.id} handed to ${"a sales executive"} — lead notified.`,
                      )
                    }
                  >
                    Take over
          ...
          <SectionCard title="Call minutes" description="Current billing cycle">
            ...
            <p className="text-xs text-muted-foreground">
              of {num(callingTotals.minutesQuota)} minutes used
            </p>
            <div className="mt-3">
              <Meter value={minutesPct} tone={minutesPct > 85 ? "amber" : "blue"} />
            </div>
              ...
              return (
                <article key={c.id} className="surface flex flex-col p-4">
                  ...
                  <p className="mt-2 text-xs text-muted-foreground">
                    Window {c.window} · {c.requiresApproval}
                  </p>
                  ...
                  <div className="mt-3 flex gap-2">
                    <Button
                      ...
                      onClick={() =>
                        toast.success(
                          c.status === "Running"
                            ? `${c.name} paused.`
                            : `${c.name} start requested — sent for approval.`,
                        )
                      }
                    >
                      {c.status === "Running" ? (
                    ...
                    <Button
                      ...
                      variant="outline"
                      onClick={() =>
                        toast("Script editor opened — changes need approval before going live.")
                      }
                    >
                      Edit script
                    </Button>
          ...
          <TabsContent value="calls" className="mt-4 space-y-4">
            <div className="surface flex flex-wrap items-center gap-2 p-3">
              <Input
                ...
                placeholder="Search lead or call ID"
              ...
              <Select value={outcome} onValueChange={setOutcome}>
                ...
                <SelectContent>
                  ...
                  {outcomeBreakdown.map((o) => (
                    <SelectItem key={o.outcome} value={o.outcome}>
                      {o.outcome}
                    </SelectItem>
                  ))}
                </SelectContent>
                ...
                {outcomeBreakdown.map((o) => {
                  ...
                  return (
                    <div key={o.outcome}>
                      ...
                      <div className="mt-1">
                        <Meter
                          value={Math.round((o.count / max) * 100)}
                          tone={OUTCOME_TONE[o.outcome] === "red" ? "red" : "blue"}
                        />
                      </div>
                    </div>
                  );
