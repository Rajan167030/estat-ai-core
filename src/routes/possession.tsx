...
export const Route = createFileRoute("/possession")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Handover pipeline from construction and snagging through final payment, possession readiness, handover and RWA transition." },
      { property: "og:title", content: "Possession & Handover — Estatum ERP" },
      { property: "og:description", content: "Stage-wise possession pipeline with snag counts and handover dates." },
...
function PossessionPage() {
  ...
  return (
    <AppShell>
      <PageHeader title="Possession" subtitle="Construction → Snagging → Final payment → Possession ready → Handover → RWA" />

      <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard label="Units in pipeline" value={String(possessionItems.length)} icon={KeyRound} accent />
          ...
          return (
            <div key={stage} className="surface flex flex-col">
              <header className="flex items-center justify-between border-b border-border px-3 py-2.5">
                ...
                <span className="num rounded-md bg-muted px-1.5 py-0.5 text-xs font-semibold">{items.length}</span>
              </header>
                  ...
                  items.map((p) => (
                    <article key={p.id} className="rounded-md border border-border p-2.5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {p.snags > 0 ? <StatusBadge status={`${p.snags} snags`} tone="amber" /> : null}
                      </div>
                      <p className="mt-0.5 truncate text-xs text-muted-foreground">{p.customer}</p>
                      <p className="truncate text-xs text-muted-foreground">{projectName(p.projectId)}</p>
                      <div className="mt-2">
